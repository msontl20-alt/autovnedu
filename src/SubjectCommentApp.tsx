...
export default function SubjectCommentApp() {
  ...
  const detectMappings = (h: string[]) => {
    ...
    h.forEach((col) => {
      ...
      // Recognizes: GK1, CK1, GK2, CK2, HK1, HK2, Giua Ky 1, Cuoi Ky 2, etc.
      const pMatch = clean.match(/(gk1|ck1|gk2|ck2|hk1|hk2|giua ky 1|cuoi ky 1|giua ky 2|cuoi ky 2)/);
      let periodInName: "GK1" | "CK1" | "GK2" | "CK2" | null = null;
      
      if (pMatch) {
        const found = pMatch[0];
        if (found.includes('gk1') || found.includes('giua ky 1')) periodInName = "GK1";
        else if (found.includes('ck1') || found.includes('hk1') || found.includes('cuoi ky 1')) periodInName = "CK1";
        else if (found.includes('gk2') || found.includes('giua ky 2')) periodInName = "GK2";
        else if (found.includes('ck2') || found.includes('hk2') || found.includes('cuoi ky 2')) periodInName = "CK2";
      }
      
      if (periodInName) lastDetectedPeriod = periodInName;

      // 2. Global Fields
      if (clean.includes('ho va ten') || clean.includes('ho ten') || clean === 'ten' || clean.includes('ten hoc sinh') || clean === 'hoten') {
        if (!mapping.name) mapping.name = col;
      }
      else if (clean.includes('ma hoc sinh') || clean.includes('ma so') || clean === 'mshs' || clean === 'id' || clean === 'ma hs' || clean.includes('ma dinh danh') || clean.includes('so hieu')) {
        if (!mapping.studentId) mapping.studentId = col;
      }
      else if (clean.includes('gioi tinh') || clean === 'nu' || clean === 'nam/nu' || clean === 'nam' || clean === 'phai' || clean === 'gt' || clean.includes('g.tinh')) {
        if (!mapping.gender) mapping.gender = col;
      }
      else if (clean.includes('giao vien') || clean === 'gvcn' || clean === 'gv' || clean.includes('nguoi cham') || clean.includes('thay/co') || clean.includes('gv giang day')) {
      ...
      // Comment detection
      const isComment = clean.includes('nhan xet') || clean.includes('nx') || clean.includes('danh gia') || clean.includes('loi phe') || clean.includes('nhanxet') || clean.includes('noi dung');
      // Level detection
      const isLevel = !isComment && (clean.includes('muc') || clean.includes('xl') || clean.includes('xep loai') || clean === 'muc dat' || clean.includes('danh gia ky'));
      // Score detection
      const isScore = !isComment && !isLevel && (clean.includes('diem') || clean.includes('ktdk') || clean.includes('kt ') || clean.includes('kiem tra') || clean === 'ck' || clean === 'gk');
  ...
  const saveEditedStudent = () => {
    ...
    const idx = newStudents.findIndex(s => s._rowIndex === editingStudent._rowIndex);
    if (idx !== -1) {
      newStudents[idx] = editingStudent;
      setStudentsData(newStudents);
      showToast(`Đã cập nhật nhận xét cho ${editingStudent[colMapping.name]}`);
    }
          ...
          <section className={cn(
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 transition shadow-lg shadow-blue-200 active:scale-95"
              >
                <Wand2 size={18} />
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <button 
                                  ...
                                  className="p-1 text-blue-600 hover:bg-blue-50 rounded transition-all opacity-0 group-hover/name:opacity-100"
        ...
        {editingStudent && (
          <>
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, x: "-50%", y: "-50%" }}
              exit={{ opacity: 0, scale: 0.8, x: "-50%", y: "-40%" }}
              className="fixed top-1/2 left-1/2 z-[70] bg-white rounded-3xl shadow-2xl p-0 w-[95%] max-w-2xl border border-gray-100 overflow-hidden"
            >
              <div className="bg-blue-600 p-6 text-white flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-lg leading-none">{editingStudent[colMapping.name]}</h3>
                    <p className="text-blue-100 text-[10px] uppercase font-bold tracking-widest mt-1">Chỉnh sửa nhận xét môn {subject}</p>
                  </div>
                </div>
                <button onClick={() => setEditingStudent(null)} className="hover:bg-white/10 p-2 rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-8 max-h-[60vh] overflow-y-auto bg-gray-50/50">
                <div className="grid grid-cols-1 gap-6">
                  {(["GK1", "CK1", "GK2", "CK2"] as const).map(period => {
                    const pMap = colMapping.periods[period];
                    if (!pMap.comment || !selectedPeriods.has(period)) return null;
                    
                    return (
                      <div key={period} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
                        <div className="flex items-center justify-between font-bold">
                          <span className="text-xs text-gray-400 uppercase tracking-wider flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-blue-500" />
                            Kỳ học: {period}
                          </span>
                          <div className="flex items-center gap-3 text-[10px]">
                            {editingStudent[pMap.score] && <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full border border-blue-100">Điểm: {editingStudent[pMap.score]}</span>}
                            {editingStudent[pMap.level] && <span className="bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full border border-purple-100">Mức: {editingStudent[pMap.level]}</span>}
                          </div>
                        </div>
                        <textarea
                          value={editingStudent[pMap.comment] || ""}
                          onChange={(e) => {
                            setEditingStudent({
                              ...editingStudent,
                              [pMap.comment]: e.target.value
                            });
                          }}
                          ...
                          placeholder={`Nhập nhận xét cho ${period}...`}
                        />
                      </div>
                    );
                  })}
              ...
              <div className="p-6 bg-white border-t border-gray-100 flex gap-3">
                <button 
                  onClick={() => setEditingStudent(null)}
                  className="flex-1 px-6 py-3 rounded-xl font-bold bg-gray-100 text-gray-500 hover:bg-gray-200 transition-all active:scale-95 text-sm"
                >
                  Hủy bỏ
                </button>
                <button 
                  onClick={saveEditedStudent}
                  className="flex-1 px-6 py-3 rounded-xl font-bold bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all active:scale-95 flex items-center justify-center gap-2 text-sm"
                >
                  <Save size={18} />
                  Lưu thay đổi
                </button>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div 
            ...
            className={cn(
              "fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 px-6 py-3 rounded-2xl shadow-2xl border backdrop-blur-sm",
